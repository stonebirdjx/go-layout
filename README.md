...
## 版本语义
- **v0.x.y** — 无兼容性承诺。可随意进行破坏性变更，但需在发布说明中记录这些变更。对于广泛使用的库而言，无限期停留在 v0 版本是一种逃避责任的做法：依赖方理应获得某种形式的承诺。
- **v1.x.y** — 承诺内容：针对 v1.0.0 编写的代码，在后续所有 v1 版本中均可编译通过。次版本号（minor version）用于新增功能，补丁版本号（patch version）用于修复问题。绝不移除或进行不兼容的更改。
- **v2+** — 视为*不同的模块*，拥有不同的导入路径（例如 `module github.com/you/lib/v2`，导入路径为 `.../lib/v2`）。这是语义化导入版本控制（semantic import versioning），且必须遵守——如果打上了 v2 标签但模块路径中不包含 `/v2`，对于使用模块模式（module-mode）的消费者来说，该版本将无法正常工作。
